...
1. `CrepeModel` - CREPE: deep convolutional neural network (CNN) model (5 pretrained weights)
...
2. `FcnF0Model` - FCN-F0: fully convolutional network (FCN) model (3 pretrained weights)
...
Based on Ardaillon and Roebel's choice, the (current) default model for `fcn_f0.predict()` is the 
...
The third argument of :code:`fcn_f0.predict` is the DL model to use. To find the available 
...
.. autoclass:: fcn_f0.predict

.. autoclass:: fcn_f0.CrepeModel
   :members: bin_frequencies, predict

.. autoclass:: fcn_f0.FcnF0Model
